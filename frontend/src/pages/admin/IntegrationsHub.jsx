                            ...
                            <div className="p-8">
                                <div className="flex items-start justify-between mb-8">
                                    ...
                                    <div className="flex flex-col items-end gap-2 text-right">
                                        <div className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border transition-colors duration-300 ${item.status === 'Connected'
                                        ...
                                        {item.required && (
                                            <span className="bg-gradient-to-r from-orange-500 to-orange-600 text-white px-3 py-1 rounded-md text-[10px] font-black uppercase tracking-tighter shadow-md shadow-orange-100">
                                                Required API
                                            </span>
                                        )}
                                    </div>
                                ...
                                <div className="space-y-5">
                                    <div>
                                        <h3 className="text-2xl font-black text-gray-900 group-hover:text-indigo-600 transition-colors duration-300">{item.name}</h3>
                                    ...
                                    <div className="grid grid-cols-2 gap-6 py-6 border-y border-gray-100/80">
                                        <div>
                                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Service Type</p>
                                            <span className="px-2.5 py-1 bg-indigo-50/50 text-indigo-700 rounded-lg text-xs font-black border border-indigo-100/50">
                                                {item.type}
                                            </span>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Usage Cost</p>
                                            <p className="text-xs font-extrabold text-gray-700">{item.cost}</p>
                                        </div>
                                    </div>

                                    <div>
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Connected Modules</p>
                                        <div className="flex flex-wrap gap-2">
                                            {item.usedBy.map(usage => (
                                                <span key={usage} className="px-2.5 py-1 bg-white border border-gray-100 text-gray-600 rounded-md text-[10px] font-black hover:border-indigo-200 hover:text-indigo-600 transition-all cursor-default">
                                                    {usage.toUpperCase()}
                                                </span>
                                            ))}
                                        </div>
                                ...
                                <div className="mt-10">
                                    {item.status === 'Connected' ? (
                                        <button className="w-full py-3.5 bg-white border border-gray-200 rounded-xl text-sm font-black text-gray-700 hover:bg-gray-50 hover:border-gray-300 hover:shadow-lg transition-all flex items-center justify-center gap-2 group/btn">
                                            <Settings className="w-4 h-4 text-gray-400 group-hover/btn:rotate-90 transition-transform duration-500" />
                                            Manage Integration
                                        </button>
                                    ) : (
                                        <button className="w-full py-3.5 bg-indigo-600 rounded-xl text-sm font-black text-white hover:bg-indigo-700 hover:shadow-[0_10px_30px_rgba(79,70,229,0.3)] transition-all flex items-center justify-center gap-2">
