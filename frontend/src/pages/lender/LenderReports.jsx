...
export default function LenderReports() {
    ...
    const handleExportPDF = async (section) => {
        ...
        const fileName = `lender-reports-${section.toLowerCase().replace(/\s+/g, '-')}-${timestamp}.pdf`;

        doc.setFontSize(20);
        doc.text("Brickbanq Lender Report", 20, 20);
        doc.setFontSize(12);
    ...
    const handleExportExcel = (section) => {
        const timestamp = new Date().toISOString().split('T')[0];
        const fileName = `lender-reports-${section.toLowerCase().replace(/\s+/g, '-')}-${timestamp}.csv`;
        ...
        <div className="max-w-[1240px] mx-auto px-6 py-6 animate-fade-in relative z-10 w-full min-screen-available font-['Inter',sans-serif]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
                <StatCard
                    ...
                    subtext={<span className="text-emerald-500 flex items-center gap-1 font-bold"><TrendingUp size={12} /> {summary?.trends.totalCases}</span>}
                    isLoading={isRefreshing}
                />
                <StatCard
                    title="Active Cases"
                    value={summary?.activeCases}
                    subtext={<span className="text-indigo-600 font-bold">{summary?.activeCases} in progress</span>}
                    isLoading={isRefreshing}
                />
                <StatCard
                    title="Total Revenue"
                    value={summary?.totalRevenue}
                    subtext={<span className="text-emerald-500 flex items-center gap-1 font-bold"><TrendingUp size={12} /> {summary?.trends.totalRevenue}</span>}
                    isLoading={isRefreshing}
                />
                <StatCard
                ...
                <StatCard
                    ...
                    subtext={<span className="text-purple-600 font-bold">{(summary?.totalBids / summary?.totalCases || 0).toFixed(1)} per case</span>}
                    isLoading={isRefreshing}
                />
                <StatCard
                    title="Success Rate"
                    value={summary?.successRate}
                    subtext={<span className="text-emerald-500 flex items-center gap-1 font-bold"><TrendingUp size={12} /> {summary?.trends.successRate}</span>}
                    isLoading={isRefreshing}
                />
            </div>
                ...
                <div className="bg-white rounded-[20px] border border-slate-100 p-6 shadow-sm group hover:shadow-md transition-all">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button className="px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-lg text-[11px] font-bold text-slate-600 flex items-center gap-1.5 hover:bg-slate-100 transition-all shadow-sm">
                ...
                <div className="bg-white rounded-[20px] border border-slate-100 p-6 shadow-sm group hover:shadow-md transition-all">
                    ...
                    <div className="h-[220px] flex items-center justify-between gap-8">
                        ...
                        <div className="flex-1 space-y-3">
                            {[
                                { label: 'Retail Loans', value: '45%', color: 'bg-[#2D31A6]' },
                                { label: 'Commercial', value: '35%', color: 'bg-[#F97316]' },
                                { label: 'Asset Finance', value: '20%', color: 'bg-[#10B981]' }
                            ].map((item, i) => (
            ...
            <div className="bg-white rounded-[20px] border border-slate-100 p-6 shadow-sm">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button className="px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-lg text-[11px] font-bold text-slate-600 flex items-center gap-1.5 hover:bg-slate-100 transition-all">
    ...
    return (
        <div className="bg-white p-5 rounded-[22px] border border-slate-100 shadow-sm flex flex-col group hover:border-indigo-100 transition-all overflow-hidden relative">
            ...
            <div className="flex items-center gap-3">
                <button
                    onClick={onPdf}
                    className="flex-1 py-2.5 bg-[#1E40AF] text-white rounded-xl text-[13px] font-bold hover:bg-[#1e3a8a] transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95"
                >
