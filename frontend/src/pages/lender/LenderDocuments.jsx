    ...
    return (
        <div className="space-y-6 animate-fade-in pt-6 pb-6">
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-2xl font-black text-[#0F172A] mb-0.5">Document Vault</h1>
                    <p className="text-[#64748B] text-[11px] font-bold uppercase tracking-wider">Secure enterprise storage for MIP case files</p>
                </div>
                <div className="flex gap-3">
                    <button className="bg-[#4F46E5] text-white px-5 py-2.5 rounded-xl text-[11px] font-black flex items-center gap-2 hover:bg-indigo-700 transition-all shadow-lg active:scale-95">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden min-h-[500px] flex flex-col">
                <div className="p-5 sm:p-6 border-b border-gray-50 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400">
                        ...
                        <span className="text-[#4F46E5]">Active Cases</span>
                    </div>
                ...
                <div className="flex-1 p-5 sm:p-10 grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-6">
                    {[1, 2, 3, 4, 5, 6].map(i => (
                        <div key={i} className="group cursor-pointer">
                            <div className="aspect-square bg-gray-50 rounded-2xl border border-gray-50 flex items-center justify-center text-gray-300 group-hover:bg-indigo-50 group-hover:border-indigo-100 group-hover:text-[#4F46E5] transition-all shadow-sm">
                                <Folder size={40} strokeWidth={1.5} className="group-hover:scale-110 transition-transform" />
                            </div>
                            <p className="text-center mt-3 text-[11px] font-black text-[#0F172A] tracking-tight truncate px-2">Case MIP-2026-00{i}</p>
...
function StorageCard({ label, value, used, icon, color, bg }) {
    return (
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all">
            ...
            <p className="text-2xl font-black text-[#0F172A] tracking-tighter leading-none mb-1">{value}</p>
            <p className="text-[#64748B] text-[10px] font-black uppercase tracking-widest mb-1">{label}</p>
