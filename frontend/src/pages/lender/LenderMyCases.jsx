...
export default function LenderMyCases() {
    ...
    const stats = useMemo(() => [
        ...
        { label: "Active Cases", value: cases.filter(c => c.status === "Active").length, icon: <Eye size={20} />, color: "text-emerald-600", bg: "bg-emerald-50" },
        { label: "In Auction", value: cases.filter(c => c.status === "In Auction").length, icon: <RefreshCw size={20} />, color: "text-rose-600", bg: "bg-rose-50" },
        { label: "Completed", value: cases.filter(c => c.status === "Completed").length, icon: <CheckCircle2 size={20} />, color: "text-purple-600", bg: "bg-purple-50" }
    ], [cases]);
        ...
        <div className="max-w-[1240px] mx-auto px-6 py-6 animate-fade-in font-['Inter',sans-serif]">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                ...
                <button
                    ...
                    className="px-5 py-2.5 bg-[#1E40AF] text-white rounded-xl text-[13px] font-bold hover:bg-[#1e3a8a] transition-all active:scale-95 shadow-sm flex items-center gap-2"
                >
