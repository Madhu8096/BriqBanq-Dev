...
export default function InvestorDashboard() {
    ...
    const firstName = profile?.firstName || (profile?.name || "").split(' ')[0] || user?.firstName || (user?.name || "").split(' ')[0] || "User";
    ...
    const stats = useMemo(() => {
        const totalInvestedValue = investments.reduce((acc, inv) => acc + (inv.currentBid || inv.targetAmount || 0), 0) / 1000000;

        const returnAmounts = investments.map(inv => ((inv.currentBid || inv.targetAmount || 100000) * (0.05 + ((inv.currentBid || 1) % 10) / 100)));
        ...
        const activeDeals = investments.filter(i => i.status !== 'Sold' && i.status !== 'Settled');
        const completedDeals = investments.filter(i => i.status === 'Sold' || i.status === 'Settled');

        const activeDealsCount = investments.length > 0 ? activeDeals.length : 14;
        const completedDealsCount = investments.length > 0 ? completedDeals.length : 3;

        const calculatedRisk = 8.5 - (completedDealsCount * 0.5) - (activeDealsCount * 0.1);
        const riskScore = investments.length > 0 ? Math.max(1, Math.min(10, calculatedRisk)).toFixed(1) : "4.2";
