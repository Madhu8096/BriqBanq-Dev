...
export default function InvestorReports() {
    ...
    const handleExportPDF = async (section) => {
        ...
        const fileName = `reports-${section.toLowerCase().replace(/\s+/g, '-')}-${timestamp}.pdf`;

        doc.setFontSize(20);
        doc.text("Brickbanq Investor Report", 20, 20);
        doc.setFontSize(12);
        doc.text(`Section: ${section}`, 20, 30);
        doc.text(`Date Range: ${dateRange}`, 20, 37);
        ...
        if (section === "Financial Summary") {
            doc.autoTable({
                ...
                body: [
                    ...
                    ['Active Cases', summary?.activeCases || '0'],
            ...
            doc.autoTable({
                ...
                head: [['Metric', 'Value']],
                body: [
                    ['Total Cases', summary?.totalCases || '0'],
                    ['Success Rate', summary?.successRate || '0%'],
                    ['Total Bids', summary?.totalBids || '0']
                ]
            });
        } else {
            doc.text("Detailed breakdown for this section is currently being processed.", 20, 60);
        }
    ...
    const handleExportExcel = (section) => {
        const timestamp = new Date().toISOString().split('T')[0];
        const fileName = `reports-${section.toLowerCase().replace(/\s+/g, '-')}-${timestamp}.csv`;
        ...
        if (section === "Financial Summary") {
            data = [
                ...
                ['Avg Case Value', summary?.avgCaseValue || ''],
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in relative z-10 w-full min-h-screen">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
                <StatCard
                    ...
                    subtext={<span className="text-green-600 flex items-center gap-1"><TrendingUp size={12} /> {summary?.trends.totalCases}</span>}
                    isLoading={isRefreshing}
                />
                <StatCard
                    title="Active Cases"
                    value={summary?.activeCases}
                    subtext={<span className="text-blue-600">{summary?.activeCases} in progress</span>}
                    isLoading={isRefreshing}
                />
                <StatCard
                    title="Total Revenue"
                    value={summary?.totalRevenue}
                    subtext={<span className="text-green-600 flex items-center gap-1"><TrendingUp size={12} /> {summary?.trends.totalRevenue}</span>}
                    isLoading={isRefreshing}
                />
                <StatCard
                    title="Avg Case Value"
                    value={summary?.avgCaseValue}
                ...
                <StatCard
                    ...
                    subtext={<span className="text-green-600 flex items-center gap-1"><TrendingUp size={12} /> {summary?.trends.successRate}</span>}
                    isLoading={isRefreshing}
                />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <div className="bg-white rounded-[16px] border border-gray-100 p-6 shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button className="px-3 py-1.5 border border-gray-200 rounded-md text-[11px] font-bold text-gray-700 flex items-center gap-1.5 hover:bg-gray-50 transition-colors shadow-sm bg-white">
                            ...
                            <div className="flex items-center w-full gap-8">
                                <div className="relative w-32 h-32 flex items-center justify-center">
                                    <svg viewBox="0 0 36 36" className="w-full h-full transform -rotate-90">
                                        {charts.revenueDistribution.reduce((acc, item, i) => {
                                            const total = 100;
...
function StatCard({ title, value, subtext, isLoading }) {
    return (
        <div className="bg-white p-6 rounded-[22px] border border-slate-100 shadow-sm flex flex-col justify-between h-[130px] relative overflow-hidden group hover:border-indigo-100 hover:shadow-md transition-all duration-300">
            <div>
...
function ReportCard({ icon, iconBg, title, description, onPdf, onExcel }) {
    return (
        <div className="bg-white p-8 rounded-[28px] border border-slate-100 shadow-sm flex flex-col h-full group hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-500 border-b-4 hover:border-b-indigo-500">
            ...
            <div className="flex items-center gap-4 mt-auto">
                <button
                    onClick={onPdf}
                    className="flex-1 py-3 bg-[#4F46E5] text-white rounded-[14px] text-xs font-bold uppercase tracking-wider hover:bg-[#4338CA] transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95 group/btn shadow-indigo-100"
                >
...
function ActivityRow({ title, description, time, isLast = false }) {
    return (
        <div className={`p-5 bg-white border border-slate-100 rounded-[20px] flex justify-between items-center transition-all hover:border-indigo-100 hover:shadow-md hover:translate-x-1 group ${!isLast ? 'mb-2' : ''}`}>
            <div>
                <h4 className="font-bold text-[14px] text-slate-900 mb-0.5 tracking-tight group-hover:text-indigo-600 transition-colors uppercase">{title}</h4>
...
function ChartSkeleton() {
    return (
        <div className="w-full h-full flex items-end justify-between gap-4 px-2 py-4">
            {[1, 2, 3, 4, 5, 6].map(i => (
                <div key={i} className="flex-1 bg-gray-50 animate-pulse rounded-t-lg" style={{ height: `${Math.random() * 60 + 20}%` }}></div>
            ))}
        </div>
    );
}
