        ...
        <div className="bg-white border border-gray-100 rounded-[20px] overflow-hidden flex flex-col shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
                <div className="flex flex-col gap-0.5">
                    <h3 className="text-[15px] font-bold text-[#1e293b]">Case Messages</h3>
                    ...
                    <div key={msg.id} className={`flex gap-3 ${msg.isMe ? 'flex-row-reverse' : ''}`}>
                        ...
                        <div className={`flex flex-col max-w-[75%] ${msg.isMe ? 'items-end' : ''}`}>
                            <div className={`flex items-baseline gap-2 mb-1.5 ${msg.isMe ? 'flex-row-reverse' : ''}`}>
                                <span className="text-[12px] font-bold text-[#1e293b]">{msg.user}</span>
                                <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{msg.role}</span>
                            </div>
                            <div className={`px-4 py-2.5 rounded-[18px] text-[13px] font-medium leading-relaxed font-sans shadow-sm ${msg.isMe ? 'bg-[#4F46E5] text-white rounded-tr-none' : 'bg-[#F1F5F9] text-[#1E293B] rounded-tl-none'
                                }`}>
                                {msg.message}
                            </div>
                            <span className="text-[9px] text-gray-400 font-medium mt-1 uppercase tracking-wide">{msg.time}</span>
                        </div>
                    </div>
                ))}
            </div>
            <div className="p-4 bg-white border-t border-gray-100">
                <div className="bg-[#F8FAFC] border border-gray-200 rounded-[16px] p-1.5 flex items-center gap-2 shadow-inner">
                    <textarea
                        rows={1}
                    ...
                    <div className="flex gap-1.5 pr-1">
                        ...
                        <button
                            ...
                            className="bg-[#1e3a8a] text-white p-2.5 rounded-[12px] hover:bg-black transition-all shadow-md active:scale-95 flex items-center justify-center group"
                        >
