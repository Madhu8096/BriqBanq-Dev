...
function DocViewModal({ doc, onClose }) {
  ...
  const hasRealFile = !!doc._localFile
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-auto bg-gray-100 min-h-0">
          {hasRealFile && isPdf(doc.name) && objectUrl && (
            <iframe
              src={objectUrl}
              title={doc.name}
          ...
          {hasRealFile && isImage(doc.name) && objectUrl && (
...
export default function DocumentsTab({ documents: initialDocuments = [], onDocumentUploaded }) {
  ...
  const handleDownload = (doc) => {
    ...
    // For mock docs there's no real file to download; the button is hidden for them below.
  }
                ...
                <tr key={doc.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {/* Download — only for real uploaded files */}
                      {doc._localFile && (
                        <button
                          type="button"
