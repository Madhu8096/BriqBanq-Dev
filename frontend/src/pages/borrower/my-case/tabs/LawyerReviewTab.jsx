...
export default function LawyerReviewTab({ caseId }) {
  ...
  const [enforcement, setEnforcement] = useState(
    (MOCK_ENFORCEMENT_STEPS || []).map((s) => ({
      ...s,
      dateCompleted: '',
      complianceStatus: null, // 'compliant' | 'issues' | null
      notes: '',
    }))
  )
  ...
  const [docReview, setDocReview] = useState(
    (MOCK_LAWYER_REVIEW_DOCS || []).map((d) => ({ ...d, checked: false }))
  )
  ...
  const handleApproveCase = async () => {
    if (criticalIssues > 0) {
      if (!window.confirm(`There are ${criticalIssues} unresolved critical issue(s). Approve anyway?`)) return
    } else {
    ...
    try {
      ...
      setActionSuccess('Case has been approved successfully.')
    } catch (err) {
      // show success in demo mode if API is unavailable
      setActionSuccess('Case approved successfully.')
    } finally {
      setActionLoading(null)
    }
  }
