...
const GROUPS_META = MOCK_SETTLEMENT_GROUPS.map(g => ({ id: g.id, title: g.title }))
...
export default function SettlementTab({ settlement: settlementProp, caseId }) {
  const summary = settlementProp && typeof settlementProp.completed === 'number'
    ? settlementProp : MOCK_SETTLEMENT_TASK_SUMMARY

  // Flatten all tasks into a single state map: id -> task
  const [taskMap, setTaskMap] = useState(() => {
    const map = {}
    const initGroups = settlementProp?.groups || MOCK_SETTLEMENT_GROUPS
  ...
  const allGroups = MOCK_SETTLEMENT_GROUPS.map(g => ({
    ...g,
    ...
    <div className="space-y-6">
      ...
      {subTab === 'pexa' && (
        <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
          ...
          <p className="text-sm text-gray-500">Connect to PEXA to manage electronic settlement. Case: <strong>{caseId || 'MIP-2026-001'}</strong></p>
