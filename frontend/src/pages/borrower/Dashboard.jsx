    ...
    const load = async () => {
      try {
        ...
        if (s && typeof s === 'object') {
          setStats({
            propertyValue: toNum(s.propertyValue ?? s.property_value),
            outstandingDebt: toNum(s.outstandingDebt ?? s.outstanding_debt),
            documentsRemaining: toNum(s.documentsRemaining ?? s.documents_remaining),
            documentsTotal: toNum(s.documentsTotal ?? s.documents_total),
            unreadMessages: toNum(s.unreadMessages ?? s.unread_messages)
          })
          const id = s.caseId ?? s.case_id
          if (id) {
            setCaseId(id)
            try { localStorage.setItem('borrowerCaseId', String(id)) } catch { /* ignore */ }
          }
          if (s.property && typeof s.property === 'object') setProperty({
            address: s.property.address ?? '',
            location: s.property.location ?? '',
            bedrooms: toNum(s.property.bedrooms),
            bathrooms: toNum(s.property.bathrooms),
            landSize: toNum(s.property.landSize ?? s.property.land_size)
          })
        }
