...
export default function NewCase({ onClose, onSuccess }) {
  ...
  const addTrustee = () => {
    const n = trustees.length + 1
    setTrustees((prev) => [...prev, { id: Date.now(), name: `Trustee ${n}` }])
    update('trusteesCount', n)
  }
  const addGuarantor = () => {
    const n = guarantors.length + 1
    setGuarantors((prev) => [...prev, { id: Date.now(), name: `Guarantor ${n}` }])
    update('guarantorsCount', n)
  }
          ...
          <div className="p-5 sm:p-6 space-y-6">
            ...
            {formData.entityType === 'Trust' && (
              <section className="rounded-lg border border-[#E9E5FF] bg-[#F5F3FF] p-4 sm:p-5">
                ...
                {trustees.length > 0 ? (
                  <ul className="mt-4 space-y-2">
                    {trustees.map((t) => (
                      <li key={t.id} className="flex items-center justify-between py-2 px-3 bg-white rounded border border-slate-200 text-sm">
                        <span className="font-medium text-slate-700">{t.name}</span>
                        <button type="button" onClick={() => { setTrustees((p) => { const next = p.filter((x) => x.id !== t.id); update('trusteesCount', next.length); return next; }); }} className="text-red-600 hover:underline text-xs">Remove</button>
                      </li>
                    ))}
                  </ul>
                ) : (
            ...
            <section className="rounded-lg border border-[#BBF7D0] bg-[#F0FDF4] p-4 sm:p-5">
              ...
              {guarantors.length > 0 ? (
                <ul className="mt-4 space-y-2">
                  {guarantors.map((g) => (
                    <li key={g.id} className="flex items-center justify-between py-2 px-3 bg-white rounded border border-slate-200 text-sm">
                      <span className="font-medium text-slate-700">{g.name}</span>
                      <button type="button" onClick={() => { setGuarantors((p) => { const next = p.filter((x) => x.id !== g.id); update('guarantorsCount', next.length); return next; }); }} className="text-red-600 hover:underline text-xs">Remove</button>
                    </li>
                  ))}
                </ul>
              ) : (
