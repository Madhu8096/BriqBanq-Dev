...
const NAV_ITEMS = [
  { name: 'Dashboard', path: 'dashboard', icon: 'dashboard', color: 'text-[#3474E1]' },
  { name: 'Assigned Cases', path: 'assigned-cases', icon: 'cases', color: 'text-blue-400' },
  { name: 'E-Signatures', path: 'e-signatures', icon: 'sign', color: 'text-[#00C853]' },
      ...
      {NAV_ITEMS.map((item) => (
        <NavLink
          ...
          className={({ isActive }) => {
            ...
            if (overlayMode) {
              return `${base} py-3.5 -mx-4 px-4 ${isActive ? 'bg-[#3474E1] text-white' : 'text-gray-600 hover:bg-gray-50'}`
            }
            return `${base} px-3 py-2 mx-2 my-0.5 rounded ${isActive ? 'bg-[#3474E1] text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'}`
          }}
        >
          {({ isActive }) => (
            <>
      ...
      >
        <div className="px-4 py-4 border-b border-gray-800">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-[#3474E1] rounded flex items-center justify-center flex-shrink-0">
