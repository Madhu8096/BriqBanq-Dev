...
export default function ESignatures() {
  ...
  const certificates = Array.isArray(data?.certificates) ? data.certificates : (Array.isArray(MOCK_GOVSIGN_CERTIFICATES) ? MOCK_GOVSIGN_CERTIFICATES : [])
  ...
  const adminPolicies = Array.isArray(data?.adminPolicies) ? data.adminPolicies : (Array.isArray(MOCK_GOVSIGN_ADMIN_POLICIES) ? MOCK_GOVSIGN_ADMIN_POLICIES : [])
  ...
  const handleDownloadEnvelope = (envelope) => {
    if (!envelope) return
    const name = `envelope-${(envelope.id || 'unknown').replace(/\s+/g, '_')}.pdf`
    triggerFileDownload(name, `Envelope: ${envelope.title}\nID: ${envelope.id}\nType: ${envelope.type}\nStatus: ${envelope.status}\nProgress: ${envelope.progress || '—'}\nSLA: ${envelope.sla || '—'}\n\n(GovSign placeholder download)`)
    setEnvelopeActionsOpen(null)
  }
  ...
  const handleExportLedger = () => {
    const name = `evidence-ledger-${new Date().toISOString().slice(0, 10)}.json`
    const content = JSON.stringify({ chainIntegrity: evidenceChain.chainIntegrity, totalEvents: evidenceChain.totalEvents, lastEvent: evidenceChain.lastEvent, events: filteredEvidence }, null, 2)
  ...
  const handleCertificateExport = (certId) => {
    const cert = certificatesDisplay.find((c) => c.id === certId)
    if (!cert) return
    const name = `certificate-${(cert.subject || certId).replace(/\s+/g, '_').slice(0, 40)}.pem`
    triggerFileDownload(name, `-----BEGIN CERTIFICATE-----\n(placeholder export for ${cert.subject})\n-----END CERTIFICATE-----`, 'application/x-pem-file')
  }
  const handleCertificateView = (cert) => () => setCertificateDetail(cert)
  const handleCertificateRenew = (certId) => {
    const cert = certificatesDisplay.find((c) => c.id === certId)
    if (cert) window.alert(`Renewal requested for certificate: ${cert.subject}. You will be notified when the new certificate is ready.`)
  }
  ...
  const handleDownloadReport = (reportId) => {
    ...
    if (!r) return
    const name = `report-${(r.name || r.id || 'report').replace(/\s+/g, '_')}.pdf`
    triggerFileDownload(name, `Report: ${r.name}\nType: ${r.type}\nDate range: ${r.dateFrom} — ${r.dateTo}\nGenerated: ${r.generatedAt}\nStatus: ${r.status}\n\n(GovSign placeholder download)`)
  }
            ...
            <div className="p-6 space-y-6">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <div className={`w-14 h-14 rounded-full flex items-center justify-center text-white font-bold text-xl shrink-0 ${
                        card.color === 'blue' ? 'bg-[#3474E1]' : card.color === 'orange' ? 'bg-amber-500' : card.color === 'purple' ? 'bg-purple-500' : 'bg-red-500'
                      }`}>
                        {card.value}
                      </div>
                      <div>
              ...
              <div>
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                  ...
                  <button type="button" onClick={handleViewAllEnvelopes} className="text-[#3474E1] text-sm font-medium hover:underline">
                    View All Envelopes
                  </button>
                </div>
                      ...
                      <div key={task.id} className="rounded-lg border border-gray-200 p-4 flex flex-wrap items-center justify-between gap-4 hover:bg-gray-50/50">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          {isSigned ? (
                            ...
                            <button type="button" onClick={handleReviewSign(task)} className="px-4 py-2 bg-[#3474E1] text-white text-sm font-medium rounded-lg hover:bg-[#2a5fc4] flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${alert.type === 'certificate' ? 'bg-amber-100' : 'bg-blue-100'}`}>
                          {alert.type === 'certificate' ? (
                            ...
                            <svg className="w-5 h-5 text-[#3474E1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                          )}
                        </div>
                        <div>
            ...
            <div className="p-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={handleCreateEnvelope} className="px-4 py-2.5 bg-[#3474E1] text-white text-sm font-medium rounded-lg hover:bg-[#2a5fc4] flex items-center gap-2">
                    <span>+</span> Create Envelope
                  </button>
                  <button
                    type="button"
                    onClick={() => setShowEnvelopeFilters((v) => !v)}
                    className={`px-4 py-2.5 border rounded-lg text-sm font-medium flex items-center gap-2 transition-colors ${showEnvelopeFilters ? 'border-[#3474E1] bg-[#EEF4FF] text-[#3474E1]' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
                  >
                    ...
                    {(envelopeTypeFilter !== 'All Types' || envelopeStatusFilter !== 'All Statuses' || envelopeRiskFilter !== 'All Risks') && (
                      <span className="w-2 h-2 rounded-full bg-[#3474E1]" />
                    )}
                  </button>
                </div>
              </div>
              {showEnvelopeFilters && (
                <div className="flex flex-wrap gap-3 mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <div className="flex items-center gap-2">
                    ...
                    <select value={envelopeTypeFilter} onChange={(e) => setEnvelopeTypeFilter(e.target.value)} className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#3474E1]">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select value={envelopeStatusFilter} onChange={(e) => setEnvelopeStatusFilter(e.target.value)} className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#3474E1]">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select value={envelopeRiskFilter} onChange={(e) => setEnvelopeRiskFilter(e.target.value)} className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#3474E1]">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-2">
                              <div className="w-20 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                <div className="h-full bg-[#3474E1] rounded-full" style={{ width: `${(row.progressVal || 0) * 100}%` }} />
                              </div>
          ...
          {activeTab === 'Documents' && (
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="rounded-xl border-2 border-gray-200 bg-white p-5 flex items-center gap-4">
                  <div className="w-14 h-14 rounded-full bg-blue-100 flex items-center justify-center shrink-0">
                    <svg className="w-7 h-7 text-[#3474E1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                  </div>
                  <div>
                ...
                <div className="rounded-xl border-2 border-gray-200 bg-white p-5 flex items-center gap-4">
                  <div className="w-14 h-14 rounded-full bg-[#E6F0FF] flex items-center justify-center shrink-0">
                    <svg className="w-7 h-7 text-[#3474E1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-gray-500 text-[#3474E1]">In progress</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <button type="button" onClick={handleUploadDocument} className="px-4 py-2.5 bg-[#3474E1] text-white text-sm font-medium rounded-lg hover:bg-[#2a5fc4] flex items-center gap-2 shadow-sm">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="relative flex-1 min-w-0">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3474E1] focus:border-transparent"
                  />
                </div>
                <select value={documentTypeFilter} onChange={(e) => setDocumentTypeFilter(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#3474E1]">
                  <option>All Types</option>
                  <option>PDF</option>
                  <option>DOCX</option>
                </select>
                <select value={documentStatusFilter} onChange={(e) => setDocumentStatusFilter(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#3474E1]">
                  <option>All Statuses</option>
                ...
                <button
                  ...
                  className={`px-4 py-2.5 border rounded-lg text-sm font-medium flex items-center gap-2 transition-colors ${
                    (documentTypeFilter !== 'All Types' || documentStatusFilter !== 'All Statuses')
                      ? 'border-[#3474E1] bg-[#EEF4FF] text-[#3474E1]'
                          ...
                          <tr key={doc.id} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center shrink-0">
                                  <svg className="w-4 h-4 text-[#3474E1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                                </div>
                            ...
                            <td className="px-6 py-4 text-right">
                              <div className="flex items-center justify-end gap-1">
                                <button type="button" onClick={() => handleDocumentView(doc)} className="p-2 text-gray-500 hover:text-[#3474E1] hover:bg-[#E6F0FF] rounded-lg transition-colors" title="View"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg></button>
                                <button type="button" onClick={() => handleDocumentDownload(doc)} className="p-2 text-gray-500 hover:text-[#3474E1] hover:bg-[#E6F0FF] rounded-lg transition-colors" title="Download"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg></button>
          ...
          {activeTab === 'Templates' && (
            <div className="p-6">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <button type="button" onClick={handleCreateTemplate} className="px-4 py-2 bg-[#3474E1] text-white text-sm font-medium rounded-lg hover:bg-[#2a5fc4] flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTemplates.map((t) => (
                  <div key={t.id} className="rounded-xl border-2 border-gray-200 p-5 hover:border-[#3474E1]/30 transition-colors flex flex-col">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="w-10 h-10 rounded-lg bg-[#E6F0FF] flex items-center justify-center shrink-0">
                        <svg className="w-5 h-5 text-[#3474E1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-gray-100">
                      <button type="button" onClick={() => handleUseTemplate(t.id)} className="px-3 py-1.5 bg-[#3474E1] text-white text-sm font-medium rounded-lg hover:bg-[#2a5fc4]">Use Template</button>
          ...
          {activeTab === 'Certificates & Keys' && (
            <div className="p-6">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <button type="button" onClick={handleIssueCertificate} className="px-4 py-2 bg-[#3474E1] text-white text-sm font-medium rounded-lg hover:bg-[#2a5fc4] flex items-center gap-2">
                  ...
                  <div key={cert.id} className={`rounded-xl border-2 p-5 ${cert.status === 'Expiring' ? 'border-amber-300 bg-amber-50/30' : 'border-green-200 bg-white'}`}>
                    ...
                    <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-gray-200">
                      ...
                      {cert.status === 'Expiring' && (
                        <button type="button" onClick={() => handleCertificateRenew(cert.id)} className="px-3 py-1.5 bg-[#3474E1] text-white text-xs font-medium rounded-lg hover:bg-[#2a5fc4]">Renew</button>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {activeTab === 'Evidence Ledger' && (
            <div className="p-6">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <button type="button" onClick={handleExportLedger} className="px-4 py-2 bg-[#3474E1] text-white text-sm font-medium rounded-lg hover:bg-[#2a5fc4]">
                  Export Ledger Proof
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                <div className="relative flex-1 min-w-[200px]">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3474E1] focus:border-transparent"
                  />
                </div>
                <select
                  value={evidenceTypeFilter}
                  onChange={(e) => setEvidenceTypeFilter(e.target.value)}
                  className="border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#3474E1]"
                >
              ...
              <div className="rounded-lg border-2 border-gray-200 bg-gray-50/50 p-6 mb-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                  <div>
                    ...
                    <select value={reportTypeFilter} onChange={(e) => setReportTypeFilter(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#3474E1]">
                      {reportTypes.map((r) => (
                  ...
                  <div className="pb-[20px]">
                    <button
                      ...
                      className="w-full px-4 py-2 bg-[#3474E1] text-white text-sm font-medium rounded-lg hover:bg-[#2a5fc4] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    >
                      {generatingReport ? (
                        <>
            ...
            <div className="p-6 space-y-6">
                  ...
                  <div key={item.id} className="flex flex-wrap items-center justify-between gap-4 py-3 border-b border-gray-100 last:border-0">
                    ...
                    {!item.locked && 'enabled' in item && (
                      <button
                        ...
                        className={`relative w-11 h-6 rounded-full transition-colors ${adminToggles[item.id] ? 'bg-[#3474E1]' : 'bg-gray-200'}`}
                      >
              ...
              <div className="rounded-lg border border-gray-200 p-5">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                  ...
                  <button type="button" onClick={handleNewPolicyRule} className="px-4 py-2 bg-[#3474E1] text-white text-sm font-medium rounded-lg hover:bg-[#2a5fc4] flex items-center gap-2">
                  ...
                  {adminSecurity.map((s) => (
                    <div key={s.id} className="flex flex-wrap items-center justify-between gap-4 py-3 border-b border-gray-100">
                      ...
                      <button
                        ...
                        className={`relative w-11 h-6 rounded-full transition-colors ${adminToggles[s.id] ? 'bg-[#3474E1]' : 'bg-gray-200'}`}
                      >
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {helpLinks.map((link) => (
                      <a key={link.id} href={link.url} className="block rounded-lg border border-gray-200 p-4 hover:border-[#3474E1]/30 hover:bg-[#E6F0FF]/50 transition-colors">
                  ...
                  <div className="mt-6 rounded-lg border-2 border-gray-200 p-4">
                    ...
                    <p className="text-sm text-gray-700">Email: <a href="mailto:support@govsign.example" className="text-[#3474E1] hover:underline">support@govsign.example</a></p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-auto">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={handleSubmitCreateEnvelope} className="flex-1 bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Create</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <div className="flex gap-2 mt-4">
              ...
              <button type="button" onClick={handleConfirmSign} className="flex-1 bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Sign</button>
            </div>
          </div>
        </div>
      )}
      ...
      {certificateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <button type="button" onClick={handleCloseCertificateModal} className="mt-4 w-full bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Close</button>
          </div>
        </div>
      )}
      ...
      {sovereigntyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <button type="button" onClick={() => setSovereigntyModal(false)} className="mt-4 w-full bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Close</button>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleSubmitCreateTemplate} className="mt-4 space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button type="submit" className="flex-1 bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Create</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <label className={`flex flex-col items-center justify-center gap-2 w-full border-2 border-dashed rounded-xl px-4 py-6 cursor-pointer transition-colors ${selectedUploadFile ? 'border-[#3474E1] bg-[#EEF4FF]/40' : 'border-gray-300 hover:border-[#3474E1] hover:bg-[#EEF4FF]/20'}`}>
                  {selectedUploadFile ? (
                    <>
                      <svg className="w-8 h-8 text-[#3474E1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                      <span className="text-sm font-medium text-[#3474E1]">{selectedUploadFile.name}</span>
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#3474E1] ${uploadNameError ? 'border-red-400' : 'border-gray-300'}`}
                />
              ...
              <div>
                ...
                <select value={uploadDocumentForm.type} onChange={(e) => setUploadDocumentForm((p) => ({ ...p, type: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#3474E1]">
              ...
              <div>
                ...
                <select value={uploadDocumentForm.envelopeId || ''} onChange={(e) => setUploadDocumentForm((p) => ({ ...p, envelopeId: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#3474E1]">
            ...
            <div className="flex gap-3 mt-6 pt-4 border-t border-gray-100">
              ...
              <button
                ...
                className="flex-1 bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2.5 rounded-lg flex items-center justify-center gap-2"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                Upload Document
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={handleSaveEditTemplate} className="flex-1 bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Save</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="view-envelope-title">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 max-h-[90vh] overflow-auto">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={() => setViewingEnvelope(null)} className="flex-1 bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Close</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="view-document-title">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 max-h-[90vh] overflow-auto">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={() => setViewingDocument(null)} className="flex-1 bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Close</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="issue-certificate-title" onClick={closeIssueCertificateModal}>
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={handleSubmitIssueCertificate} className="flex-1 bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Issue Certificate</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="new-policy-rule-title" onClick={closeNewPolicyRuleModal}>
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={handleSubmitNewPolicyRule} className="flex-1 bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Add Rule</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="hsm-modal-title">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-auto">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={() => setShowHSMModal(false)} className="flex-1 bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Save Configuration</button>
            </div>
          </div>
        </div>
      )}
      ...
      {certificateDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 max-h-[90vh] overflow-auto">
            ...
            <button type="button" onClick={() => setCertificateDetail(null)} className="mt-4 w-full bg-[#3474E1] hover:bg-[#2a5fc4] text-white text-sm font-medium py-2 rounded-lg">Close</button>
          </div>
        </div>
      )}
    </div>
