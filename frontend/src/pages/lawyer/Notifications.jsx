...
import { MOCK_NOTIFICATIONS, MOCK_NOTIFICATION_STATS } from './data/mockData'
  ...
  if (type === 'bid' || type === 'system') {
    return (
      <div className={`${base} bg-blue-50 border border-blue-200`}>
        <svg className="w-6 h-6 text-[#3474E1]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function Notifications() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold text-[#2A3037]">Notifications</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-[#2A3037]">{unread}</p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-[#2A3037]">{notifications.length}</p>
            <p className="text-xs text-gray-500 mt-0.5">All time</p>
          </div>
          <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center">
            <svg className="w-6 h-6 text-[#3474E1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-500">This Week</p>
            <p className="text-2xl font-bold text-[#2A3037]">{MOCK_NOTIFICATION_STATS.thisWeek}</p>
            <p className="text-xs text-gray-500 mt-0.5">Last 7 days</p>
          </div>
          <div className="w-12 h-12 rounded-full bg-green-100 flex items-center justify-center">
            <svg className="w-6 h-6 text-[#00C853]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button type="button" onClick={handleClearFilters} className="px-3 py-2 text-sm font-medium text-[#2A3037] hover:underline">
          Clear Filters
        </button>
        <button type="button" onClick={handleMarkAllRead} className="px-3 py-2 rounded-lg bg-gray-100 text-[#2A3037] text-sm font-medium hover:bg-gray-200 flex items-center gap-1">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-[#2A3037]">Notifications ({notifications.length})</h2>
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((item) => (
            <div
              ...
              className={`rounded-xl border p-4 cursor-pointer ${item.unread ? 'bg-[#E6F0FF]/50 border-[#3474E1]/20' : 'bg-white border-gray-200'} shadow-sm`}
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex gap-3 flex-1 min-w-0">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                    ...
                    {item.type === 'bid' ? (
                      <svg className="w-5 h-5 text-[#3474E1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
                    ) : (
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-medium text-[#2A3037]">{item.title}</p>
                      {item.unread && (
                        <span className="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-[#3474E1] text-white">New</span>
                      )}
                    </div>
                    <p className="text-sm text-gray-600 mt-0.5">{item.description}</p>
                    <div className="flex flex-wrap items-center gap-3 mt-2" onClick={(e) => e.stopPropagation()}>
                      <button type="button" onClick={() => handleViewDetails(item)} className="text-sm font-medium text-[#3474E1] hover:underline flex items-center gap-1">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                        View Details
                      </button>
                      {item.unread && (
                        <button type="button" onClick={() => handleMarkRead(item.id)} className="text-sm font-medium text-[#3474E1] hover:underline flex items-center gap-1">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                          Mark as read
                        </button>
                      )}
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-start justify-between gap-3 p-6 pb-4 shrink-0">
              <div className="flex items-start gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  <h2 id="lawyer-notification-detail-title" className="text-lg font-semibold text-[#2A3037]">{selectedNotification.title}</h2>
              ...
              <div className="flex items-center gap-2 shrink-0">
                {selectedNotification.unread ? (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-300 bg-white text-[#2A3037] text-sm font-medium hover:bg-gray-50"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                    Mark as read
                  </button>
            ...
            <div className="px-6 pb-6 space-y-4 overflow-y-auto flex-1">
              <div>
                ...
                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-sm font-medium bg-[#3474E1] text-white">
                  {getTypeLabel(selectedNotification.type)}
                </span>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Message</label>
                <div className="rounded-lg border border-gray-200 bg-gray-100 px-3 py-2.5 text-sm text-[#2A3037]">
              ...
              <div>
                ...
                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-sm font-medium bg-blue-50 text-[#3474E1] border border-blue-200">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-[#2A3037]">{formatTimeDisplay(selectedNotification.time) || '—'}</p>
              </div>
              ...
              <div className="rounded-lg bg-blue-50 border border-blue-100 p-4 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-sm font-semibold text-[#2A3037]">{getRelatedItem(selectedNotification.type).label}</p>
                  <p className="text-xs text-gray-600 mt-0.5">Click &quot;Go to Related Item&quot; to view this item</p>
                </div>
                <button type="button" onClick={handleGoToRelatedItem} className="p-1.5 text-[#3474E1] hover:bg-blue-100 rounded shrink-0" aria-label="View related item">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50 shrink-0">
              <button type="button" onClick={handleCloseDetailModal} className="px-4 py-2 border border-gray-300 bg-white text-[#2A3037] text-sm font-medium rounded-lg hover:bg-gray-50">
                Close
              </button>
              <div className="flex items-center gap-2">
                <button type="button" onClick={handleGoToRelatedItem} className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#3474E1] hover:bg-[#2a5fc7] text-white text-sm font-medium rounded-lg">
