...
const STORAGE_KEY = 'brickbanq_lawyer_profile'
...
const LawyerProfileContext = createContext(null)

export const useLawyerProfile = () => {
  const ctx = useContext(LawyerProfileContext)
  if (!ctx) return { profile: defaultProfile, updateProfile: () => {} }
  return ctx
}

export const LawyerProfileProvider = ({ children }) => {
  ...
  return (
    <LawyerProfileContext.Provider value={{ profile, updateProfile }}>
      {children}
