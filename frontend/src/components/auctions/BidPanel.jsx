 ...
 */
export default function BidPanel({ currentBid = 0, placeBid }) {
  const [amount, setAmount] = useState("");
  const minIncrement = 10000;
  const buyerPremiumRate = 0.02; // Update to 2% as per screenshot

  // Derived Values
  const recommendedBid = (currentBid || 0) + minIncrement;
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <p className="text-[10px] text-gray-400 mt-2 font-medium">
              Minimum increment: {formatCurrency(minIncrement)} • Current bid: {formatCurrency(currentBid)}
            </p>
          </div>

          {/* Quick Buttons */}
          <div className="grid grid-cols-3 gap-3">
            <QuickBidButton label="+A$10k" onClick={() => handleQuickAdd(10000)} />
            <QuickBidButton label="+A$25k" onClick={() => handleQuickAdd(25000)} />
            <QuickBidButton label="+A$50k" onClick={() => handleQuickAdd(50000)} />
          </div>
