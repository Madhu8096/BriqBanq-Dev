...
export default function AdminDealCard({ deal }) {
  const countdown = deal?.auctionEnd ? useCountdown(deal.auctionEnd) : null;
  ...
  const getStatusConfig = (status) => {
    switch (status) {
      case "Live Auction": return { badge: "bg-red-600", text: "LIVE AUCTION" };
      case "Coming Soon": return { badge: "bg-blue-600", text: "COMING SOON" };
      case "Buy Now":
      case "Active": return { badge: "bg-green-600", text: "BUY NOW" };
      case "Sold": return { badge: "bg-slate-900", text: "SOLD" };
      default: return { badge: "bg-gray-500", text: status?.toUpperCase() };
    }
  };
  ...
  const getTargetRoom = (id, status) => {
    if (status === "Buy Now" || status === "Active") return `/admin/buy-now/${id}`;
    return `/admin/auction-room/${id}`;
  };
  ...
  return (
    ...
    >
      <div className="relative h-64 overflow-hidden">
        <img
          src={deal.image || "https://images.unsplash.com/photo-1560518883-ce09059eeffa?q=80&w=1000"}
          alt={deal.title}
      ...
      <div className="p-6 flex flex-col flex-1 space-y-4">
        <div className="space-y-1 pb-4 border-b border-gray-50">
          <h3 className="text-xl font-semibold text-gray-900 tracking-tight group-hover:text-indigo-600 transition-colors">{deal.title}</h3>
        ...
        <div className="space-y-2.5">
          ...
          {deal.status === "Live Auction" && <ValueRow label="Current Bid" value={formatCurrency(deal.currentBid)} color="text-green-600" bold />}
          {(deal.status === "Buy Now" || deal.status === "Active") && (
            <ValueRow label="Fixed Price" value={formatCurrency(deal.buyNowPrice || deal.loanAmount)} color="text-green-600" bold />
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col justify-end space-y-4">
          {deal.status === "Coming Soon" && (
            <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl p-3 flex justify-between items-center">
              <div className="flex items-center gap-2 text-indigo-600"><Calendar size={16} /><span className="text-xs font-bold uppercase tracking-wider">Auction</span></div>
          ...
          {deal.status === "Live Auction" && (
            <div className="bg-red-50 border border-red-100 rounded-xl p-3 flex justify-between items-center">
              <div className="flex items-center gap-2 text-red-600"><Clock size={16} /><span className="text-xs font-bold uppercase tracking-wider">Ends in</span></div>
              <span className="text-xs font-bold text-red-600">{countdown?.formatted || "Soon"}</span>
            </div>
          )}
          <div className="flex justify-between items-center pt-2">
            <span className="text-[10px] font-bold text-gray-300 uppercase tracking-widest">ID: {deal.id}</span>
            {deal.totalBids > 0 && <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{deal.totalBids} bids</span>}
          </div>
          <div className="flex gap-2">
            {deal.status === "Live Auction" ? (
              <>
                <button onClick={(e) => { e.stopPropagation(); navigate(`/admin/auction-room/${deal.id}`); }}
              ...
              <div className="flex w-full gap-2">
                <button onClick={(e) => { e.stopPropagation(); navigate(getTargetRoom(deal.id, deal.status)); }}
                  ...
                  <div className="flex items-center justify-center gap-2"><Eye size={14} />View</div>
                </button>
                {(deal.status === "Buy Now" || deal.status === "Active") && (
                  <button onClick={(e) => { e.stopPropagation(); navigate(`/admin/buy-now/${deal.id}`); }}
