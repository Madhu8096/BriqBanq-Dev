...
export default function IntegrationsHub() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    // Simulate fetch; replace with: getIntegrationsHub().then(...)
    Promise.resolve({
      integrations: MOCK_INTEGRATIONS,
      metrics: MOCK_INTEGRATIONS_METRICS,
      categories: MOCK_INTEGRATION_CATEGORIES,
    })
      .then((data) => {
        if (cancelled) return
  ...
  const handleModalConnect = async (integration, formData) => {
  ...
  const handleConfigureSave = async (integration, config) => {
    ...
    // await integrationApi.updateIntegrationConfig(integration.id, config)
    handleConfigureModalClose()
  }

  const handleConfigureTestConnection = async (integration) => {
  ...
  if (loading && !integrations.length) {
    return (
      <div className="space-y-6 max-w-full min-w-0">
        ...
        <IntegrationMetrics metrics={MOCK_INTEGRATIONS_METRICS} />
  ...
  return (
    <div className="space-y-6 max-w-full min-w-0">
      ...
      <IntegrationMetrics metrics={metrics || MOCK_INTEGRATIONS_METRICS} />

      <IntegrationCategoryFilters
        categories={categories.length ? categories : MOCK_INTEGRATION_CATEGORIES}
