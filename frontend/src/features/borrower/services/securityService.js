...
export const securityService = {
  async getSecuritySettings() {
    try {
      ...
      if (err?.code === 'ERR_NETWORK' || err?.isOffline) {
        ...
        return cached || { twoFaEnabled: false, sessions: MOCK_ACTIVE_SESSIONS }
      }
      const cached = settingsStorage.getSecurity()
  ...
  async changePassword(data) {
    try {
      ...
      // Don't cache password changes for security
      throw err
    }
  },
  ...
  async toggleTwoFactor(enabled) {
    try {
      const res = await borrowerApi.toggleTwoFactor(enabled)
  ...
  async getActiveSessions() {
    try {
      ...
      if (err?.code === 'ERR_NETWORK' || err?.isOffline) {
        const cached = settingsStorage.getSecurity()
        return cached?.sessions || MOCK_ACTIVE_SESSIONS
      }
      const cached = settingsStorage.getSecurity()
      return cached?.sessions || MOCK_ACTIVE_SESSIONS
    }
