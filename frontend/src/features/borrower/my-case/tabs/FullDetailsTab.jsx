import { MOCK_CASE_DATA as MOCK_BORROWER_CASE } from '../../data/borrowerMockData'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-gray-200 rounded-lg p-5">
          ...
          <dl className="space-y-2 text-sm">
            <div><dt className="text-gray-500">Case Created</dt><dd className="font-medium text-gray-900">{c.caseCreated ?? '10 Jan 2026, 05:30'}</dd></div>
            <div><dt className="text-gray-500">Last Updated</dt><dd className="font-medium text-gray-900">{c.lastUpdated ?? '10 Feb 2026, 05:30'}</dd></div>
            <div><dt className="text-gray-500">Urgency Level</dt><dd className="font-medium text-gray-900">{c.urgencyLevel ?? 'High'}</dd></div>
            <div><dt className="text-gray-500">Total Bids</dt><dd className="font-medium text-gray-900">{c.totalBids ?? 7} bids received</dd></div>
            <div><dt className="text-gray-500">Current Highest Bid</dt><dd className="font-medium text-green-600">{formatNum(c.currentHighestBid ?? 1100000)}</dd></div>
          </dl>
        </div>
        <div className="bg-white border border-gray-200 rounded-lg p-5">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">Financial Overview & Risk Assessment</h3>
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
            <div><dt className="text-gray-500">Property Valuation</dt><dd className="font-medium text-green-600">{formatNum(c.property?.valuation ?? 1250000)}</dd></div>
            <div><dt className="text-gray-500">Outstanding Debt</dt><dd className="font-medium text-red-600">{formatNum(c.lender?.outstandingDebt ?? 980000)}</dd></div>
            <div><dt className="text-gray-500">LVR (Loan to Value)</dt><dd className="font-medium text-gray-900">{c.lvr ?? 78.4}%</dd></div>
            <div><dt className="text-gray-500">Arrears</dt><dd className="font-medium text-red-600">{formatNum(c.activeArrears ?? c.financials?.arrearsAndInterest ?? 18500)}</dd></div>
            <div><dt className="text-gray-500">Equity Available</dt><dd className="font-medium text-green-600">{formatNum(c.equityAvailable ?? 270000)}</dd></div>
            <div><dt className="text-gray-500">Minimum Bid</dt><dd className="font-medium text-gray-900">{formatNum(c.minimumBid ?? 1000000)}</dd></div>
          </dl>
        </div>
      </div>
