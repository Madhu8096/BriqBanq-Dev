...
import { auctionService, userService } from "../../api/dataService";
...
export default function InvestorAuctionRoom() {
  ...
  useEffect(() => {
    ...
    const fetchAuctionData = async () => {
      ...
      try {
        ...
        if (!res.success) {
          setError(res.error || "The requested investment opportunity could not be found.");
          return;
        }

        const data = res.data;
        if (!data) {
          setError("Asset not found or empty.");
          return;
        }
  ...
  const handlePlaceBid = async (amount) => {
    try {
      // Optimistic Update / Success Simulation
      const res = await auctionService.placeBid(id, amount);
      if (res.success) {
        // Local dynamic feedback using returned data if available
        const newBid = res.data?.bid || { user: "You", amount, time: "Just now" };
                  ...
                  <div>
                    ...
                    <p className="text-sm text-indigo-500 font-medium mt-1">
                      {isNotified
                        ...
                        : "Bidding will open on Mar 12, 2026."}
                    </p>
                  </div>
                  <button
                    onClick={() => {
