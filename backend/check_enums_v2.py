...
async def check_enum():
    ...
    output = []
    output.append(f"Connecting to: {settings.database_url}")
    ...
    async with engine.connect() as conn:
        ...
        output.append(f"Current 'case_status' enum values: {labels}")
        ...
        output.append(f"Current 'deal_status' enum values: {labels_deal}")
    
    await engine.dispose()
    
    with open("enum_check_results.txt", "w") as f:
        f.write("\n".join(output))
